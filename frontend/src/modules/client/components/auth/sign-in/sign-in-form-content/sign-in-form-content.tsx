  ...
  return (
    <Form>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'end' }}>
        <StyledLink to={ClientRouteEnum.PASSWORD_RECOVERY}>Forgot password?</StyledLink>
      </Box>
      <Divider sx={{ mt: 3 }}>Or with</Divider>
      <Box sx={{ display: 'flex', justifyContent: 'space-around' }}>
        <GoogleAuthButton userType={UserTypesEnum.CLIENT} disabled={isSubmitting} />
      </Box>
      <Button
        sx={{ mt: 6, mb: 3 }}
        type="submit"
      ...
      <Typography sx={{ textAlign: 'center', mb: 10 }}>
        ...
        <StyledLink to={ClientRouteEnum.SIGN_UP}> Sign Up</StyledLink>
      </Typography>
    </Form>
  );
};
